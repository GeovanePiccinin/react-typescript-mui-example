...
function MoviesPage() {
  ...
  const trendingList = onlyMovies.filter((item) => item.isTrending === true);
  const recommendList = onlyMovies.filter((item) => item.isTrending !== true);

  const handleSearch = (e: { target: { value: SetStateAction<string> } }) => {
    setSearch(e.target.value);
    const newList = onlyMovies.filter((movie) =>
